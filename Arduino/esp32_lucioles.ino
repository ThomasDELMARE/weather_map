...
#ifdef TLS_USE
int MQTT_PORT =  1883; // for TLS cf https://test.mosquitto.org/
#else
int MQTT_PORT = 1883;
#endif
...
void loop () {
  ...
  // NEW 
  if(getCooler()==1){
    jsondoc["status"]["ledCooler"] = "HIGH";
  }
  else{
    jsondoc["status"]["ledCooler"] = "LOW";  
  }

  if(getHeater()== 1){
    jsondoc["status"]["ledHeater"] = "HIGH";
  }
  else{
    jsondoc["status"]["ledHeater"] = "LOW";  
  }
  ...
  jsondoc["info"]["loc"] = "43.599690; 7.091460";
  // Thomas
  jsondoc["info"]["user"] = "22006617";
  // Marie-Céleste
  // jsondoc["info"]["user"] = "22106761";
  // Hasnaa
  // jsondoc["info"]["user"] = "21808389";
  jsondoc["info"]["uptime"] = String(uptime::getSeconds());
  jsondoc["info"]["ssid"] = return_wifi_status()["ssid"];
  jsondoc["info"]["ident"] = return_wifi_status()["mac"];
  jsondoc["info"]["ip"] = return_wifi_status()["ip"];

  jsondoc["reporthost"]["target_ip"] = MQTT_SERVER;
  jsondoc["reporthost"]["target_port"] = MQTT_PORT;
  jsondoc["reporthost"]["sp"] = String(period);
  
  jsondoc["regul"]["threshold"] = light_threshold;
  jsondoc["regul"]["sbn"] = sbn;
  jsondoc["regul"]["shn"] = shn;
  jsondoc["regul"]["sbj"] = sbj;
  jsondoc["regul"]["shj"] = shj;
  

  serializeJson(jsondoc, payload); // On transforme le Json en string
